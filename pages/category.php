...
if (!$invalidCsrf && (1 === (int) filter_input(INPUT_POST, 'btn_save') || 1 === (int) filter_input(INPUT_POST, 'btn_apply'))) {
    ...
    $category = new D2U_Partner\Category($form['category_id']);
...
if ((!$invalidCsrf && 1 === (int) filter_input(INPUT_POST, 'btn_delete', FILTER_VALIDATE_INT)) || 'delete' === $func) {
    ...
    $uses_partners = $category->getPartners();

    if (0 === count($uses_partners)) {
        $category->delete();
    } else {
        $message = '<ul>';
        foreach ($uses_partners as $uses_partner) {
            $message .= '<li><a href="index.php?page=d2u_partner/partner&func=edit&entry_id='. $uses_partner->partner_id .'">'. rex_escape($uses_partner->name).'</a></li>';
        }
        $message .= '</ul>';
				...
				<fieldset>
					...
					<div class="panel-body-wrapper slide">
						<?php
                            ...
                            $category = new D2U_Partner\Category($entry_id);
...
if ('' === $func) {
    $query = 'SELECT category_id, name '
        . 'FROM '. rex::getTablePrefix() .'d2u_partner_categories ';
